...
import {
  ...
  RotationTracker,
  TranslationTracker,
} from "./movement-trackers";
import { iterateNumericEnumKeyedRecord } from "@/app/utils";

export class ModelMovementManager {
  public activeTrackers: Partial<
    Record<ModelMovementType, ModelMovementTracker>
  > = {};
  ...
  constructor(public transformNode: TransformNode) {
    transformNode.rotationQuaternion = Quaternion.FromEulerVector(
  ...
  smoothLookAtThenLockOn() {
    ...
    const forward = targetPos
      .subtract(this.transformNode.getAbsolutePosition())
      .normalize();

    const up = Vector3.Up();
    const targetRotation: Quaternion = Quaternion.FromLookDirectionLH(
      forward,
      up
    );
